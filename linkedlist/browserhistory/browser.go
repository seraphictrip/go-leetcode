...
func (bh BrowserHistory) String() string {
	return "{" + bh.history[bh.index] + "}"
}

func Constructor(homepage string) BrowserHistory {
	history := make([]string, 1, 1024)
	history[0] = homepage
...
func (bh *BrowserHistory) Current() string {
	return bh.history[bh.index]
}

// void visit(string url) Visits url from the current page. It clears up all the forward history.
func (bh *BrowserHistory) Visit(url string) {
	bh.history = append(bh.history[0:bh.index+1], url)
	fmt.Println(bh.history)
	bh.index = len(bh.history) - 1
}
